#!/bin/python
# written by Chang Park(naltang@yahoo.com) 2011.9.20
...
def read_sam_file(file_in, callback):
...
def read_fasta_file(file_in, callback):
...
def main():
    ...
    if(len(sys.argv) != 4):
        print "Usage: sam2fasta.py [ref.fasta] [in.sam] [out.fasta]"
        print "where:"
    ...
    read_fasta_file(file_in_ref, on_ref)

    if(g_count_error != 0):
        sys.stderr.write("Error, in input files\n")
        return

    read_sam_file(file_in_sam, on_sam)
